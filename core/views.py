...
class CategoryPostsView(TemplateView):
    ...
    def get(self, request, pk):
        try:
            article = Article.objects.select_related('author', 'category').get(pk=pk)  # fetching the article
        except Article.DoesNotExist:
            return HttpResponseNotFound("Article not found")

        is_bookmarked = is_scheduled = upvoted = downvoted = False
        if request.user.is_authenticated:
            is_bookmarked = Favourite.objects.filter(user=request.user, post=article).exists()
            is_scheduled = ReadLater.objects.filter(user=request.user, post=article).exists()
        ...
        comments = Comment.objects.select_related('user').filter(article=article).order_by("-created_at")
        paginator = Paginator(comments, 5)
        page_number = self.request.GET.get("page", 1)
        comments_page = paginator.get_page(page_number)

        if request.htmx:
            return render(request, "article/cmts.html", context={"comments": comments_page})
    ...
    def post(self, request, pk, *args, **kwargs):
        if request.htmx and request.headers.get("src") == "comment":
            ...
            user_vote, created = Vote.objects.get_or_create(user=request.user, article=article, defaults={'vote_type': src})
            ...
            return render(request, "partials/ratings.html", {"article": article, "upvoted": upvoted, "downvoted": downvoted})
            ...
            if request.headers.get("src") == "bookmark":
                ...
                return render(request, "partials/bookmarked.html", context={"article": article})
            else:
            ...
            if request.headers.get("src") == "schedule":
                ...
                return render(request, "partials/scheduled.html", context={"article": article})
            else:
                ReadLater.objects.filter(user=request.user, post=article).delete()
                return render(request, "partials/schedule.html", context={"article": article})
